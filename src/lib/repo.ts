...
export async function create<T>(collectionId: string, data: any) {
  ...
  return (await databases.createDocument(databaseId, collectionId, ID.unique(), enriched)) as unknown as T;
}
...
export async function update<T>(collectionId: string, documentId: string, data: any) {
  ...
  return (await databases.updateDocument(databaseId, collectionId, documentId, enriched)) as unknown as T;
}
